...
const BarChart = ({ data, dataKey = 'value', xAxisKey = 'name', color = '#5856D6', height = 300, showGrid = true, showTooltip = true, showXAxis = true, showYAxis = true, animate = true }) => {
        ...
        <div className={`p-2 rounded shadow-lg ${isDarkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-800'} border ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
          ...
          <p className="text-sm">
            <span className="font-medium" style={{ color }}>
              {dataKey === 'price' || dataKey === 'value' ? '$' : ''}{payload[0].value.toLocaleString()}
            </span>
          </p>
        </div>
      );
        ...
        >
          ...
          {showYAxis && (
            <YAxis
              ...
              axisLine={{ stroke: gridColor }}
              tickLine={{ stroke: gridColor }}
              tickFormatter={(value) => dataKey === 'price' || dataKey === 'value' ? `$${value}` : value}
            />
          )}
